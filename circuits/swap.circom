...
template Swap(n, m, levels) {
   ...
   /*  Generate commitment if operation will be successful */
    ...
    for (var i = 0; i < n; i++) {
        hasher3.ai_list[i] <== ai_list_new_fail[i];
    }
    ...
    lte[0].in[0] <== 0;
    lte[0].in[1] <== ai_list_old[0] + ai_deltas[0] - fee;
    lte[0].out === 1;
    ...
    for (var i = 1; i < n; i++) {
        ...
        lte[i].in[0] <== 0;
        lte[i].in[1] <== ai_list_old[i] + ai_deltas[i];
        lte[i].out === 1;
...
component main = Swap(100, 170, 20);
